...
def G1DListCrossoverPMX(genome, **args):
   """ The PMX Crossover for G1DList  (partially matched crossover) """
   sister = None
   brother = None
   gMom = args["mom"]
   gDad = args["dad"]
   ...
   if args["count"] >= 1:
      sister = gMom.clone()
      sister.resetStats()
      for i, index in zip(range(c1, c2), range(c1, c2)):
         for j in range(listSize - 1):
            if sister.genomeList[j] == gDad.genomeList[index]:
               break
         sister.genomeList[i], sister.genomeList[j] = sister.genomeList[j], sister.genomeList[i]
   ...
   if args["count"] == 2:
      ...
      for i, index in zip(range(c1, c2), range(c1, c2)):
         for j in range(listSize - 1):
            if brother.genomeList[j] == gMom.genomeList[index]:
               break
         brother.genomeList[i], brother.genomeList[j] = brother.genomeList[j], brother.genomeList[i]
...
def G1DListCrossoverCX(genome, **args):
   """ The CX Crossover for G1DList  (cycle crossover) """
   sister = None
   brother = None
   gMom = args["mom"]
   gDad = args["dad"]
   ...
   mask = [False for i in range(listSize)]
   mask[0] = True
   current = 0
   while (gDad[current] != gMom[0]):
      for i in range(0, listSize):
         if gMom[i] == gDad[current]:
            mask[i] = True
            current = i
            break
   for i in range(0, listSize):
   ...
   mask[0] = True
   current = 0
   while (gMom[current] != gDad[0]):
      for i in range(0, listSize):
         if gDad[i] == gMom[current]:
            mask[i] = True
            current = i
            break
   for i in range(0, listSize):
